...
class ContactUsListViewController: NANavigationViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    var societyServiceArray = [NAString().digi_gate(), NAString().plumber(), NAString().carpenter(), NAString().electrician(), NAString().garbage_Collection(), NAString().emergency(), NAString().event_management()]
