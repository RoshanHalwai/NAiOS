...
class RetrievingApartmentServicesData {
    ...
    private func getOwnerData(ownerUID: String, callback: @escaping (_ dailyServiceData: NammaApartmentDailyServices) -> Void) {
        ...
        dailyServiceDataRef.observeSingleEvent(of: .value) { (dataSnapshot) in
            ...
            let dailyServiceData = NammaApartmentDailyServices(fullName: (fullName as! String), phoneNumber: phoneNumber as? String, profilePhoto: profilePhoto as? String, providedThings: "", dateOfHandedThings: "", rating: rating as? Int, timeOfVisit: nil, uid: (uid as! String), type: nil, numberOfFlat: self.flatCount, status: nil, userUID: "")
